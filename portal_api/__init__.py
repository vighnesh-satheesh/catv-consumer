...
class AppInit:
    ...
    def __init__(self):
        env = os.environ.get("CATV_CONSUMER_API_ENV")
        if env is None or env not in ["development", "production"]:
            raise AttributeError(
                "Missing environment variable 'CATV_CONSUMER_API_ENV'."
                "CATV_CONSUMER_API_ENV value should be either development or production."
            )
    ...
    def set_environment_variables_from_parameter_store(cls):
        ...
        if ssm_path:
            ...
            env_file = os.environ.get("CATV_CONSUMER_API_ENV_PATH")
            os.environ["PORTAL_API_MODE"] = "development"
            if env_file is None or os.path.isfile(env_file) is False:
                raise AttributeError(
                    "Please set environ variable 'CATV_CONSUMER_API_ENV_PATH' for env file path."
                    "'export CATV_CONSUMER_API_ENV_PATH=/env/file/path.env'"
                )

    @classmethod
    def set_allowed_hosts(self):
        allowed_host = []
        if os.environ.get("CATV_CONSUMER_API_ENV") != "development" or os.environ.get("CONTAINER_TYPE") == "portal_admin":
            try:
